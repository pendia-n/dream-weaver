...
interface DreamMessage { id: number; role: string; content: string; type: string; image_urls: string; created_at: string; extra_data: string | null; }
...
function renderAuth() {
  app.innerHTML = `<div class="auth-container"><div class="auth-box">
    ...
    <h1>Dreamweaver</h1>
    <p class="tagline">Interpret your dreams with AI through the lenses of history's great minds</p>
    <div class="auth-tabs">
      <button class="auth-tab active" id="tab-login">Login</button>
      <button class="auth-tab" id="tab-register">Register</button>
    </div>
    <div id="auth-form"></div>
    <div id="auth-links" style="margin-top:12px"></div>
  </div></div>`;
...
function checkUsername(username: string) {
  ...
  fetch('/api/auth/check-username?username=' + encodeURIComponent(username))
    .then(r => r.json()).then(d => { updateUsernameStatus(d.available, d.error); })
...
function renderAuthForm() {
  ...
  if (links) {
    links.innerHTML = authMode === 'login'
      ? '<a href="#" id="forgot-pw-link" style="color:var(--text-dim);font-size:0.85em">Forgot password?</a>'
      : '';
...
function navHtml(extra: string = '') {
  ...
  return `<div class="navbar">
    <div class="nav-brand" onclick="currentView='home';render()">🌙 Dreamweaver</div>
    <div class="nav-links">
      <button onclick="currentView='home';render()" class="${_currentView==='home'?'active':''}">Home</button>
      <button onclick="currentView='new-dream';render()" class="${_currentView==='new-dream'?'active':''}">New</button>
      <button onclick="currentView='history';render()" class="${_currentView==='history'?'active':''}">History</button>
      <button onclick="currentView='main';render()" class="${_currentView==='main'?'active':''}">Main</button>
      <button onclick="currentView='credits';render()" class="${_currentView==='credits'?'active':''}">Credits</button>
      <button onclick="currentView='settings';render()" class="${_currentView==='settings'?'active':''}">Settings</button>
      ${isAdmin ? '<button onclick="currentView=\'admin\';render()">Admin</button>' : ''}
      ${extra}
      <button onclick="logout()" class="btn-logout">Logout</button>
    </div>
    <div class="nav-credits">💰 ${currentUser ? currentUser.credits : 0}</div>
  </div>`;
}
...
// ═══════════════════════════════════
function renderHome() {
  app.innerHTML = navHtml() + `<div class="main-content">
    <div class="home-hero">
      <div class="hero-emoji">🌙</div>
      <h1>Welcome, ${currentUser ? currentUser.username : ''}</h1>
      <p>Your dreams hold meaning. Let AI help you understand them through the lenses of history's great minds.</p>
      <button class="btn-primary btn-large" onclick="currentView='new-dream';render()">✨ New Dream Session</button>
    </div>
    <div class="home-actions">
      <div class="action-card" onclick="currentView='history';render()">
        <div class="action-emoji">📜</div>
        <h3>History</h3>
        <p>Review past dream interpretations</p>
      </div>
      <div class="action-card" onclick="currentView='main';render()">
        <div class="action-emoji">🔮</div>
        <h3>Main Orchestrator</h3>
        <p>Chat with AI across all your dreams</p>
      </div>
      <div class="action-card" onclick="currentView='credits';render()">
        <div class="action-emoji">💰</div>
        <h3>Buy Credits</h3>
        <p>Get more credits for interpretations</p>
      </div>
    </div>
  </div>`;
}

function logout() { token = ''; currentUser = null; localStorage.removeItem('dw_token'); _currentView = 'home'; render(); }

// ═══════════════════════════════════
// NEW DREAM
// ═══════════════════════════════════
function renderNewDream() {
  const lensOptions = LENSES.map(l =>
    `<label class="lens-option"><input type="radio" name="lens" value="${l.id}" ${l.id==='jung'?'checked':''} /><span class="lens-emoji">${l.emoji}</span><span class="lens-info"><strong>${l.name}</strong><span>${l.desc}</span></span></label>`
  ).join('');
  app.innerHTML = navHtml() + `<div class="main-content">
    <div class="dream-container">
      <div class="section-header"><h2>✨ Describe Your Dream</h2><p class="hint">Write or speak your dream. Choose an interpretation lens and mood.</p></div>
      <div class="input-area">
        <textarea id="dream-input" placeholder="I was walking through a forest when I came upon a river. The water was dark but I could see something glowing beneath the surface..." rows="6"></textarea>
        <div class="input-actions"><button class="btn-voice" id="voice-btn" onclick="toggleVoice()">🎤 Voice Input</button><span id="voice-status"></span></div>
      </div>
      <div class="mood-row">
        <div class="mood-group"><label>Mood Before Sleep</label><input type="range" id="mood-before" min="1" max="10" value="5" oninput="document.getElementById('mood-before-val').textContent=this.value" /><span id="mood-before-val">5</span>/10</div>
        <div class="mood-group"><label>Mood After Waking</label><input type="range" id="mood-after" min="1" max="10" value="5" oninput="document.getElementById('mood-after-val').textContent=this.value" /><span id="mood-after-val">5</span>/10</div>
      </div>
      <div class="lens-selector">
        <label>Interpretation Lens</label>
        <div class="lens-grid">${lensOptions}</div>
      </div>
      <div class="mode-selector">
        <p>Response mode:</p>
        <div class="mode-options">
          <label class="mode-option"><input type="radio" name="dream-mode" value="text_only" checked /><div class="mode-info"><strong>Text Only</strong><span>2 credits — AI interpretation text</span></div></label>
          <label class="mode-option"><input type="radio" name="dream-mode" value="text_and_images" /><div class="mode-info"><strong>Text + 2 Images</strong><span>3 credits — Interpretation + 2 dream images</span></div></label>
        </div>
      </div>
      <button class="btn-primary btn-large" id="submit-dream" onclick="submitDream()">Interpret Dream</button>
      <p id="dream-error" class="error"></p>
    </div>
  </div>`;
}
...
function toggleVoice() {
  ...
  if (!SR) { alert('Speech recognition not supported. Please type your dream.'); return; }
  ...
  recognition.onresult = function(e: any) { let t = ''; for (let i = 0; i < e.results.length; i++) t += e.results[i][0].transcript; (document.getElementById('dream-input') as HTMLTextAreaElement).value = t; document.getElementById('voice-status')!.textContent = '🔴 Recording... (click to stop)'; };
...
async function submitDream() {
  ...
  try {
    ...
    currentMessages = [
      { id: 0, role: 'user', content: text, type: '', image_urls: '', created_at: '', extra_data: null },
      { id: 1, role: 'assistant', content: data.interpretation, type: 'interpretation', image_urls: data.image_urls ? JSON.stringify(data.image_urls) : '', created_at: '', extra_data: data.interpretation_extra || null },
    ];
...
function renderDream() {
  ...
  app.innerHTML = navHtml('<button onclick="currentView=\'new-dream\';render()">+ New</button>') +
    '<div class="main-content"><div class="dream-chat">' +
    '<div class="dream-chat-header"><h3>' + currentDream.title + '</h3></div>' +
    '<div class="messages" id="messages">' + messagesHtml + '</div>' +
    (hasImg ? '<div class="selected-image-bar"><span>📌 Selected: <img src="' + selectedImage + '" class="thumb" /></span><button onclick="selectedImage=null;render()">Clear</button></div>' : '') +
...
function selectImage(url: string) { selectedImage = selectedImage === url ? null : url; render(); }
...
async function sendChat() {
  ...
  try {
    ...
    currentUser!.credits = data.creditsLeft;
    currentMessages.push({ id: 0, role: 'user', content: text, type: '', image_urls: '', created_at: '', extra_data: null });
    if (data.image_urls?.length > 0) currentMessages = currentMessages.filter(m => m.type !== 'image_generation');
    currentMessages.push({ id: 0, role: 'assistant', content: data.response, type: 'chat', image_urls: data.image_urls ? JSON.stringify(data.image_urls) : '', created_at: '', extra_data: null });
    selectedImage = null;
...
function renderMainChat() {
  ...
  if (!area) return;
  area.innerHTML = `<div class="messages" id="main-messages"><div class="msg msg-ai"><div class="msg-content">Hello! I have access to all your dream interpretations. Ask me about patterns, themes, recurring symbols, or anything across your dreams.</div></div></div><div class="chat-input-area"><textarea id="main-input" placeholder="Ask about your dreams..." rows="3"></textarea><button class="btn-primary" onclick="sendMainChat()">Send (0.5cr)</button></div><p id="main-error" class="error"></p>`;
}

async function loadSymbols() {
  const area = document.getElementById('main-content-area');
  if (!area) return;
  area.innerHTML = '<div class="loading">Loading symbols...</div>';
  try {
    const data = await api('/symbols');
    if (data.symbols && data.symbols.length > 0) {
      let html = '<div class="symbol-grid">';
      for (const s of data.symbols) {
        html += `<div class="symbol-card"><h4>${s.symbol}</h4><p>${s.meaning || ''}</p><div class="symbol-count">Appeared ${s.count} times · First: ${new Date(s.first_seen).toLocaleDateString()}</div></div>`;
      }
      html += '</div>';
      area.innerHTML = html;
    } else {
      area.innerHTML = '<p style="color:var(--text-dim)">No symbols yet. Interpret some dreams first!</p>';
    }
...
async function loadMoods() {
  const area = document.getElementById('main-content-area');
  if (!area) return;
  area.innerHTML = '<div class="loading">Loading mood data...</div>';
  try {
    const data = await api('/moods');
    if (data.moods && data.moods.length > 0) {
      let html = '<div class="mood-chart">';
      for (const m of data.moods.slice(-14)) {
        const beforeH = (m.mood_before || 5) * 10;
        const afterH = (m.mood_after || 5) * 10;
        html += `<div class="mood-bar"><div class="bar-before" style="height:${beforeH}px"></div><div class="bar-after" style="height:${afterH}px"></div><div class="bar-label">${new Date(m.created_at).toLocaleDateString().slice(5)}</div></div>`;
      }
      html += '</div>';
      html += '<div style="font-size:0.75em;color:var(--text-dim);margin-top:8px"><span style="color:var(--primary)">■</span> Before sleep &nbsp; <span style="color:var(--success)">■</span> After waking</div>';
      area.innerHTML = html;
    } else {
      area.innerHTML = '<p style="color:var(--text-dim)">No mood data yet. Track your mood when creating dreams!</p>';
    }
  } catch (e: any) { area.innerHTML = '<p class="error">' + e.message + '</p>'; }
}
...
function renderCredits() {
  app.innerHTML = navHtml() + '<div class="main-content"><h2>💰 Credits</h2><div class="settings-section"><h3>Balance</h3><p style="font-size:2em;font-weight:700;color:var(--warning)">' + (currentUser ? currentUser.credits : 0) + ' credits</p></div><div class="settings-section"><h3>Usage</h3><p style="color:var(--text-dim);font-size:0.85em">New dream text-only: 2cr · New dream text+images: 3cr · Chat text→text: 1cr · Chat image ref→text: 2cr · Chat text→images: 3cr · Chat image ref→images: 4cr · Main chat: 0.5cr</p></div><div class="settings-section"><h3>Buy Credits</h3><div class="credit-packs"><div class="credit-pack"><h4>30 Credits</h4><p>$4.99</p><button onclick="startCheckout(\'CREDIT_30\')">Buy</button></div><div class="credit-pack"><h4>100 Credits</h4><p>$9.99</p><button onclick="startCheckout(\'CREDIT_100\')">Buy</button></div><div class="credit-pack"><h4>300 Credits</h4><p>$19.99</p><button onclick="startCheckout(\'CREDIT_300\')">Buy</button></div></div></div></div>';
}
...
function renderSettings() {
  const totpStatus = currentUser?.totpEnabled
...
async function setupTOTP() {
  try { const data = await api('/user/totp/setup', { method: 'GET' }); showTOTPSecret(data.secret, data.uri); } catch (e: any) { alert(e.message); }
}
...
async function loadReport() {
  try {
    ...
    if (data.byRegion?.length) { html += '<h4>Dreams by Region</h4><ul>'; for (const r of data.byRegion) html += '<li>' + (r.country || 'Unknown') + ': ' + r.dream_count + '</li>'; html += '</ul>'; }
    if (data.emotionAnalysis) html += '<h4>AI Analysis</h4><div class="analysis">' + data.emotionAnalysis + '</div>';
    if (data.topUsers?.length) { html += '<h4>Top Dreamers</h4><ol>'; for (const u of data.topUsers) html += '<li>' + u.username + ': ' + u.dream_count + '</li>'; html += '</ol>'; }
    html += '</div>';
